...
use BitCode\BITGFZC\Integration\ZohoCRM\RecordApiHelper;
use BitCode\BITGFZC\Integration\IntegrationHandler;
use BitCode\BITGFZC\Core\Util\IpTool;
...
final class ZohoCRMHandler
{
    ...
    {
        \add_action('wp_ajax_bitgfzc_zcrm_generate_token', array(__CLASS__, 'generateTokens'));
        \add_action('wp_ajax_bitgfzc_zcrm_refresh_modules', array(__CLASS__, 'refreshModulesAjaxHelper'));
        \add_action('wp_ajax_bitgfzc_zcrm_refresh_layouts', array(__CLASS__, 'refreshLayoutsAjaxHelper'));
    }
    ...
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bitgfzc_nonce')) {
    ...
    public static function refreshLayoutsAjaxHelper()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bitgfzc_nonce')) {
            ...
            if (!is_wp_error($layoutsMetaResponse) && (empty($layoutsMetaResponse->status) || (!empty($layoutsMetaResponse->status) && $layoutsMetaResponse->status !== 'error'))) {
                ...
                $fieldToHide = ['Lead_Source', 'Lead_Status'];
                $dataTypeToHide = ['profileimage', 'ownerlookup', 'fileupload'];
                $layouts = [];
                    ...
                    foreach ($layoutValue->sections as $sectionKey => $sectionValue) {
                        foreach ($sectionValue->fields as $fieldKey => $fieldDetails) {
                            if (
                                ...
                                && !in_array($fieldDetails->api_name, $fieldToHide)
                                && !empty($fieldDetails->view_type->create)
                                && $fieldDetails->view_type->create
                                && !in_array($fieldDetails->data_type, $dataTypeToHide)
    ...
    {
        add_filter('bitgfzc_addRelatedList', array(__CLASS__, 'addRelatedList'), 10, 6);
    }
    ...
    public function execute(IntegrationHandler $integrationHandler, $integrationData, $fieldValues)
    {
        ...
        if (
            ...
            $error = new WP_Error('REQ_FIELD_EMPTY', __('module, layout, fields are required for zoho crm api', 'bitgfzc'));
        ...
        if (
            ...
            $zcrmApiResponse = apply_filters('bitgfzc_addRelatedList', $zcrmApiResponse, $this->_formID, $entryID, $this->_integrationID, $logID, $fieldValues, $integrationDetails, $recordApiHelper);
        }
        return $zcrmApiResponse;
    }
}
